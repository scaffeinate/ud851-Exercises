...
public class NotificationUtils {
    ...
    private static final int ACTION_DRINK_WATER_PENDING_INTENT_ID = 4421;

    private static final int ACTION_DISMISS_PENDING_INTENT_ID = 4314;
    ...
    public static NotificationCompat.Action ignoreReminderAction(Context context) {
        ...
        waterReminderIntent.setAction(ACTION_DISMISS_NOTIFICATION);
        PendingIntent pendingIntent = PendingIntent.getService(context, ACTION_DISMISS_PENDING_INTENT_ID,
                waterReminderIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        return new NotificationCompat.Action(R.drawable.ic_cancel_black_24px,
                context.getResources().getString(R.string.action_dismiss), pendingIntent);
    }
    ...
    public static NotificationCompat.Action drinkWaterAction(Context context) {
        Intent waterReminderIntent = new Intent(context, WaterReminderIntentService.class);
        waterReminderIntent.setAction(ACTION_INCREMENT_WATER_COUNT);
        PendingIntent pendingIntent = PendingIntent.getService(context, ACTION_DRINK_WATER_PENDING_INTENT_ID,
                waterReminderIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        return new NotificationCompat.Action(R.drawable.ic_local_drink_black_24px,
                context.getString(R.string.action_drink_water), pendingIntent);
    }
